...
```ts
import { bind, group, key, profile, setup } from "karabiner-config-builder";
...
- `group()` groups bindings, layers, and nested condition scopes.
- `bind()` creates a Karabiner basic manipulator.
- `layer()` creates modal key layers.
- Output helpers like `key()`, `none()`, `shell()`, `script()`, `app()`, and `url()` create actions.
...
A top-level group inside a profile compiles to a Karabiner complex modification rule. Nested groups compile away into inherited conditions.
...
export default setup(
    profile(
        ...
        group(
            ...
            bind("semicolon", key("return_or_enter")),
        ),
    ),
);
```

Nested groups are useful for shared context, like device-specific bindings with app-specific subgroups:
...
const appleMagicKeyboard = fromDevice({
    vendor_id: 1452,
    product_id: 1234,
    is_keyboard: true,
...
```ts
import { bind, group, key, layer } from "karabiner-config-builder";
...
The builder supports group-level conditions as a convenience by applying them to every manipulator generated by that group.

```ts
import { bind, group, inApp, key } from "karabiner-config-builder";
...
Group-level conditions are merged with manipulator-level conditions.

Compatible list-based conditions are merged and deduped, and obvious impossible condition combinations are rejected.
...
Repack and reinstall the global CLI during local development:
